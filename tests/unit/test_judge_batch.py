...
from models.evaluation import BenchmarkResult, GroundTruth
from models.evaluation import JudgeVerdict
...
def test_judge_batch_skips_final_status(tmp_path: Path, monkeypatch) -> None:
    ...
    pending = BenchmarkResult(item_id="i1", judge_status="pending")
    done = BenchmarkResult(
        item_id="i2",
        judge_status="ok",
        judge_verdict=JudgeVerdict(judge_model="m", judge_version="v", scores={"x": 1.0}),
    )
    ...
    judge = MagicMock()
    verdict = JudgeVerdict(judge_model="mock", judge_version="v", scores={"synthesis_grounding": 0.9})
