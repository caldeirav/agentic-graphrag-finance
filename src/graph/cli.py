...
def main() -> None:
    ...
    parser.add_argument("--issuer", default="", help="CIK or issuer id")
    parser.add_argument("--ticker", default="", help="Ticker (parsed under data/parsed/{ticker}/)")
    ...
    issuer_key = args.ticker.upper() if args.ticker else args.issuer
    if not issuer_key:
        raise SystemExit("Provide --issuer or --ticker")
    issuer_dir = args.parsed_dir / issuer_key
    ...
    snapshot = build_snapshot(issuer_key, docs, snapshot_id=args.snapshot_id or None)
