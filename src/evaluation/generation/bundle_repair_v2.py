...
def repair_expected_section_paths_row(
    ...
    hints = infer_section_hints(
        ...
        gt_text=" ".join(gt.get("required_claims") or []),
    )
    ...
    if accessions:
        for acc in accessions:
            picked = pick_section_path_for_accession(acc, graph_paths, hints)
            if picked:
                repaired.append(picked)
    if not repaired:
        repaired = canonical

    resolved, still_bad = resolve_section_paths(
        repaired,
        graph_paths,
        snapshot_accessions=snapshot_accessions,
    )
...
def repair_row(
    ...
    paths, path_changed = repair_expected_section_paths_row(
        row, graph_paths, snapshot_accessions
    )
    if path_changed:
        row = dict(row)
...
def repair_bundle(
    ...
    *,
    ...
    for row in rows:
        ...
        updated, changed = repair_row(
            row, graph_paths, snapshot_accessions, bundle_version
        )
        if changed:
